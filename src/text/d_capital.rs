...
impl D {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 8
        position: self.generate_coordinates(max_x * (1.0 - text.font_wide_h), max_y * 0.80,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 9
      ...
      Vertex { // 10
        position: self.generate_coordinates(max_x, max_y * 0.12,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 11
      ...
      Vertex { // 12
        position: self.generate_coordinates(max_x * (1.0 - text.font_wide_h), max_y * 0.18,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 13
    ...
    let indices = vec![
      ...
      self.generate_indices(2, 7, 5),
      self.generate_indices(5, 7, 6),
      self.generate_indices(7, 8, 6),
      self.generate_indices(8, 12, 10),
      self.generate_indices(8, 10, 6),
      self.generate_indices(11, 10, 12),
      self.generate_indices(11, 9,  10),
      self.generate_indices(11, 13, 9),
      self.generate_indices(13, 9, 11),
      self.generate_indices(13, 4,  9),
    ].concat();
