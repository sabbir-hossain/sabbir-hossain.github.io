...
impl C {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 10
        position: self.generate_coordinates(max_x * 0.80, max_y * (1.0-text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 11
        position: self.generate_coordinates(max_x * (1.0 - text.font_wide_h), max_y * 0.82,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 12
        position: self.generate_coordinates(max_x * (1.0 - text.font_wide_h), max_y * 0.78,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 13
        position: self.generate_coordinates(max_x, max_y * 0.78,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 14
      ...
      Vertex { // 17
        position: self.generate_coordinates(max_x * (1.0 - text.font_wide_h), max_y * 0.15,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 18
        position: self.generate_coordinates(max_x * (1.0 - text.font_wide_h), max_y * 0.20,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 19
        position: self.generate_coordinates(max_x, max_y * 0.20,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ];
