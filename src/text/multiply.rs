...
impl Multiply {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 6
        position: self.generate_coordinates(max_x * (1.0-text.font_wide_h), max_y * text.font_wide_v,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 7
    ...
    let indices = vec![
      self.generate_indices(0, 6, 7), 
      self.generate_indices(0, 7, 1),
      self.generate_indices(4, 5, 2),
      self.generate_indices(2, 5, 3),
    ].concat();
