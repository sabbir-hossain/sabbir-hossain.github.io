  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 1
        position: [ 
          self.left_bottom_x, 
          self.left_bottom_y + max_y*text.font_wide_h, 
          0.0
        ], 
    ...
    let indices = vec![
      ...
      self.generate_indices(4, 5, 6), // 2
      self.generate_indices(4, 6, 3), // 2
    ].concat();
