...
impl E {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 8
        position: self.generate_coordinates(max_x * 0.22, max_y * 0.55,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 9
        position: self.generate_coordinates(max_x * 0.22,  max_y  * (0.55 - text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 10
    ...
    let indices = vec![
      ...
      self.generate_indices(4, 5, 13),
      self.generate_indices(4, 13, 12),
    ].concat();
