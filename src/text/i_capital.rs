...
impl I {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 2
        position: self.generate_coordinates(max_x * 0.65, max_y * (1.0 - text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 3
        position: self.generate_coordinates(max_x * 0.65, max_y * text.font_wide_v,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ...
    let indices = vec![
      ...
      self.generate_indices(4, 8, 5),
      self.generate_indices(5, 8, 9),
      self.generate_indices(6, 10, 7),
