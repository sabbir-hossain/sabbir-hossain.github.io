...
impl T {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 4
        position: self.generate_coordinates(max_x*(0.5-text.font_wide_h*0.5), max_y*(1.0-text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 5
        position: self.generate_coordinates(max_x*(0.5+text.font_wide_h*0.5), max_y*(1.0-text.font_wide_v),  0.0), 
    ...
    let indices = vec![
      self.generate_indices(0, 1, 3),
      self.generate_indices(0, 3, 2),
      self.generate_indices(4, 6, 7),
      self.generate_indices(4, 7, 5),
    ].concat();
