...
impl Three {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 2
        position: self.generate_coordinates(max_x*text.font_wide_h, max_y*0.7,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 3
      ...
      Vertex { // 19
        position: self.generate_coordinates(max_x*0.40, max_y * 0.54,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },  
      Vertex { // 20
        position: self.generate_coordinates(max_x*0.40, max_y * (0.54-text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 21
      ...
      Vertex { // 29
        position: self.generate_coordinates(max_x*text.font_wide_h, max_y*text.font_wide_v*2.5,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 30
        position: self.generate_coordinates(0.0, max_y*text.font_wide_v*2.5,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ];
