...
impl U {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 11
        position: self.generate_coordinates(max_x*(1.0-text.font_wide_h), max_y,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 12
    ...
    let indices = vec![
      self.generate_indices(0, 4, 1),
      self.generate_indices(1, 4, 2),
      self.generate_indices(0, 3, 4),
      self.generate_indices(3, 6, 5),
      self.generate_indices(3, 5, 4),
      self.generate_indices(6, 8, 5),
      self.generate_indices(5, 8, 7),
      self.generate_indices(7, 8, 10),
      self.generate_indices(7, 10, 9),
      self.generate_indices(9, 10, 13),
      self.generate_indices(9, 13, 11),
      self.generate_indices(11, 13, 12),
    ].concat();
