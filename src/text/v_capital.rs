...
impl V {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 6
        position: self.generate_coordinates(max_x / 2.0, max_y * text.font_wide_v * 2.0,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 7
        position: self.generate_coordinates(max_x*text.font_wide_h*2.0, 0.0,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 8
    ...
    let indices = vec![
      self.generate_indices(4, 1, 0), // 0
      self.generate_indices(4, 7, 6), // 1
      self.generate_indices(4, 6, 1), // 2
      self.generate_indices(7, 8, 6), // 3
      self.generate_indices(6, 8, 5), // 4
      self.generate_indices(2, 6, 5), // 5
      self.generate_indices(2, 5, 3), // 6
    ].concat();
