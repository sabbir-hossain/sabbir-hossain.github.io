...
impl M {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 8
        position: self.generate_coordinates(max_x * text.font_wide_v, max_y * (1.0-text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 9
        position: self.generate_coordinates(max_x * (1.0-text.font_wide_v), max_y * (1.0-text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 10
      ...
      Vertex { // 13
        position: self.generate_coordinates(max_x * text.font_wide_h * 2.0, max_y / 3.0,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 14
        position: self.generate_coordinates(max_x * (1.0-text.font_wide_h * 2.0), max_y / 3.0,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ];
    ...
    let indices = vec![
      self.generate_indices(0, 3, 2), // 0
      self.generate_indices(1, 0, 2), // 1
      self.generate_indices(2, 8, 11), // 2
      self.generate_indices(7, 4, 5), // 3
      self.generate_indices(6, 7, 5), // 4
      self.generate_indices(12, 9, 6), // 5
      self.generate_indices(8, 13, 11), // 6
      self.generate_indices(11, 13, 10), // 7
      self.generate_indices(13, 14, 10), // 8
      self.generate_indices(12, 10, 9), // 9
      self.generate_indices(10, 14, 9), // 10
    ].concat();
