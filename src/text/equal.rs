...
impl Equal {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      Vertex { // 0
        position: self.generate_coordinates(0.0, max_y*0.2,  0.0), 
      ...
      Vertex { // 2
        position: self.generate_coordinates(max_x, max_y*0.2,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      }, 
      Vertex { // 3
      ...
      Vertex { // 5
        position: self.generate_coordinates(0.0, max_y*0.8,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 6
      ...
      Vertex { // 7
        position: self.generate_coordinates(max_x, max_y*0.8,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ];
    ...
    let indices = vec![
      ...
      self.generate_indices(5, 4, 6), // 2
      self.generate_indices(5, 6, 7), // 3
    ].concat();
