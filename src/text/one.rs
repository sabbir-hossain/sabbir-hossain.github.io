...
impl One {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 1
        position: self.generate_coordinates(max_x * (1.0-text.font_wide_h*1.8), max_y,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 2
      ...
      Vertex { // 3
        position: self.generate_coordinates(max_x * (1.0-text.font_wide_h*1.8), 0.0,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 4
      ...
      Vertex { // 5
        position: self.generate_coordinates(0.0, max_y * 0.7,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 6
        position: self.generate_coordinates(max_x * (1.0-text.font_wide_h*1.5), max_y * (1.0-text.font_wide_v*1.5),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 7
        position: self.generate_coordinates(max_x * text.font_wide_h*2.0, max_y * 0.7,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ];
    ...
    let indices = vec![
      ...
      self.generate_indices(4, 5, 7),
      self.generate_indices(7, 6, 4),
      self.generate_indices(4, 6, 1),
    ].concat();
