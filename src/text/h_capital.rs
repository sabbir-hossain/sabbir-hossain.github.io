...
impl H {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 5
        position: self.generate_coordinates(max_x * text.font_wide_h, max_y  * (0.55 - text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      ...
      Vertex { // 11
        position: self.generate_coordinates(max_x * (1.0 - text.font_wide_h), max_y  * (0.55 - text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ];
    ...
    let indices = vec![
      ...
      self.generate_indices(8, 9, 6),
      self.generate_indices(6, 7, 8),
      self.generate_indices(4, 5, 11),
      self.generate_indices(4, 11, 10),
    ].concat();
