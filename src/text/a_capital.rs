...
impl A {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 2
        position: self.generate_coordinates( max_x * 0.43, max_y, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 3
        position: self.generate_coordinates( max_x * 0.2, max_y * 0.49, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 4
        position: self.generate_coordinates( max_x * 0.2, max_y * 0.37, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 5
        position: self.generate_coordinates( max_x * 0.2, 0.0, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 6
        position: self.generate_coordinates( max_x * 0.5, max_y * 0.83, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 7
        position: self.generate_coordinates( max_x * 0.57, max_y, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 8
        position: self.generate_coordinates( max_x * 0.8, max_y * 0.49, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 9
        position: self.generate_coordinates( max_x * 0.8, max_y * 0.37, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 10
        position: self.generate_coordinates( max_x * 0.8, 0.0, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
      Vertex { // 11
      ...
      Vertex { // 12
        position: self.generate_coordinates( max_x, max_y * 0.5, 0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0 
      },
    ];
  
    let indices = vec![
      self.generate_indices(0, 3,  1),
      self.generate_indices(0, 5,  3),
      self.generate_indices(1, 3,  2),
      self.generate_indices(3, 6,  2),
      self.generate_indices(6, 7,  2),
      self.generate_indices(6, 12, 7),
      self.generate_indices(8, 12, 6),
      self.generate_indices(4, 9,  3),
      self.generate_indices(3, 9,  8),
      self.generate_indices(10,11, 8),
      self.generate_indices(8, 11, 12)
    ].concat();
