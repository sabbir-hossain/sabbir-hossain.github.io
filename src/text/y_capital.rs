...
impl Y {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      ...
      Vertex { // 4
        position: self.generate_coordinates(max_x / 2.0, max_y* 0.55,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 5
        position: self.generate_coordinates(max_x*(0.5-text.font_wide_h*0.5), max_y * 0.45,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 6
        position: self.generate_coordinates(max_x*(0.5+text.font_wide_h*0.5), max_y * 0.45,  0.0), 
      ...
      Vertex { // 7
        position: self.generate_coordinates(max_x*(0.5-text.font_wide_h*0.5),0.0,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 8
        position: self.generate_coordinates(max_x*(0.5+text.font_wide_h*0.5),0.0,  0.0), 
    ...
    let indices = vec![
      self.generate_indices(0, 5, 4),
      self.generate_indices(0, 4, 1),
      self.generate_indices(5, 6, 4),
      self.generate_indices(5, 7, 6),
      self.generate_indices(7, 8, 6),
      self.generate_indices(4, 6, 3),
      self.generate_indices(4, 3, 2),
    ].concat();
