...
impl Plus {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      Vertex { // 0
        position: self.generate_coordinates(0.0, max_y*0.4,  0.0), 
      ...
      Vertex { // 2
        position: self.generate_coordinates(max_x, max_y*0.4,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      }, 
      Vertex { // 3
      ...
      Vertex { // 4
        position: self.generate_coordinates(max_x * 0.35, max_y*(1.0-text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      }, 
      Vertex { // 5
        position: self.generate_coordinates(max_x * 0.7, max_y*(1.0-text.font_wide_v),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 6
        position: self.generate_coordinates(max_x * 0.35, max_y * text.font_wide_v,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      }, 
      Vertex { // 7
        position: self.generate_coordinates(max_x * 0.7, max_y * text.font_wide_v,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ];

    let indices = vec![
      self.generate_indices(1, 0, 2), // 0
      self.generate_indices(1, 2, 3), // 1

      self.generate_indices(4, 6, 7), // 2
      self.generate_indices(4, 7, 5), // 3
    ].concat();
