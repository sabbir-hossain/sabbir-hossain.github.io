...
impl FirstBracketLeft {
  ...
  pub fn get_coordinates(mut self, text: &Text) -> TextObject {
    ...
    let vertices = vec![
      Vertex { // 0
        position: self.generate_coordinates(max_x*0.25, max_y * text.font_wide_v * 1.5,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 1
        position: self.generate_coordinates(max_x*0.25, max_y * (1.0-text.font_wide_v * 1.5),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 2
        position: self.generate_coordinates(max_x*0.45, max_y * (1.0-text.font_wide_v*0.5),  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 3
        position: self.generate_coordinates(max_x*0.45, max_y * text.font_wide_v*0.5,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 4
      ...
      Vertex { // 6
        position: self.generate_coordinates(max_x*0.75, max_y,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
      Vertex { // 7
      ...
      Vertex { // 9
        position: self.generate_coordinates(max_x*0.75, 0.0,  0.0), 
        color: text.color, tex_coords: [0.0, 0.0], mode: 0,
      },
    ];
